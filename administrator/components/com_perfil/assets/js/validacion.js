            ...
            if (car!=null&&car!="") {
             ...
             boton.children().first().text(Joomla.JText._('COM_PERFIL_SELECT_OPTION'));                          
                 ...
                 var key = $("#token").attr("name");
                    var url='index.php?option=com_perfil&task=funciones.getCar&format=raw';
                    $.ajax({
                        type:"POST",
                        url: url,
   ...
   function validateReference(value){
            ...
            $("#jform_zip_code").keypress(function(event) {
                if(event.keyCode == 13&&event.target.value.length==5) {
                    ...
                    var key = $("#token").attr("name");
                    var url='index.php?option=com_perfil&task=funciones.getZip&format=raw';
            ...
            $("#jform_zip_code").keyup(function(event){
                ...
                if(zip.length>1){
                    if(event.keyCode!=13){
                        ...
                        var key = $("#token").attr("name");
                        var url='index.php?option=com_perfil&task=funciones.getDatos&format=raw';
                        if(zip!=""||zip!=null){
                            $.ajax({
                                type:"POST",
                                url: url,
        ...
        function validateExistEmail(email){
            ...
            var url='index.php?option=com_perfil&task=funciones.validaCorreo&format=raw';
