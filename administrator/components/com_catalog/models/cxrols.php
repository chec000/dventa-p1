...
class catalogModelcxrols extends JModelList
{
    /**
     * 
     * @param string $config
    ...
    public function __construct($config = array())
    {
        if (empty($config['filter_fields']))
        {
          $config['filter_fields'] = array(
              'a.id', 'b.name','b.description','b.file_name','rolid','rol');
        }
    ...
    protected function getListQuery()
    {
        ...
        $query->select(
                $this->getState(
                        ...
                        'a.id,b.name,' .
                        'b.description,' .
                        'b.file_name,r.id as rolid,r.title as rol'
                        )
                    );
        $query->from($db->quoteName('core_product_categories_x_roles').' AS a');
        $query->join('INNER', $db->quoteName('core_product_categories', 'b') . ' ON (' . 
                    $db->quoteName('b.id') . ' = ' . 
     ...
     * @return type
     */
    public function getTable($type = 'cxrol', $prefix = 'catalogTable', 
            $config = array())
    {
    ...
    private function setSearch(&$query,&$db)
    {
        ...
        if (!empty($search))
        {
          if (stripos($search, 'id:') === 0)
          {
            ...
            $query->where('(a.category_id = '.$search.
                    ' OR a.rol_id = '.$search.')');
          }
        }
        return $query;        
