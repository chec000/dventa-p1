...
class catalogModelproducts extends JModelList
{
    /**
     * 
     * @param type $config
    ...
    protected function getListQuery()
    {
        ...
        $query->join('INNER', $db->quoteName('core_products_x_categories', 'cp') . ' ON (' . 
            $db->quoteName('cp.product_id') . ' = ' . 
            $db->quoteName('a.id') . ')');
        $query->join('LEFT', $db->quoteName('#__core_user_products_likes', 'l') . ' ON (' . 
            $db->quoteName('l.product_id') . ' = ' . 
            $db->quoteName('a.id') . ')');
        $query->where('a.enabled = 1 and a.editable = 0');        
        $query->group('a.id');
