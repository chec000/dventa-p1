...
<div id="j-main-container" class="span10">
  <form action="<?php echo JRoute::_('index.php?option=com_catalog&view=cxrols'); ?>"
      ...
      <thead>
        <tr>
          ...
          <th width="2%" class="nowrap center hidden-phone">
            <?php echo JHtml::_('grid.sort', 'VIEW_CATEGORIES_GETSORTFIELD_ID','a.id', $listDirn, $listOrder); ?>
          </th>          
          <th class="product_title" width="15%">
            <?php echo JHtml::_('grid.sort', 'CATALOGS_CATEGORY', 'b.name', $listDirn, $listOrder); ?>
          </th>
          <th lass="product_description">
            <?php echo JHtml::_('grid.sort', 'VIEW_CATEGORIES_GETSORTFIELD_DESCRIPTION', 'b.description'); ?>
          </th>        
          <th width="10%" class="product_img">
            <?php echo JHtml::_('grid.sort', 'VIEW_CATEGORIES_DEFAULT_TABLE_IMAGE','b.file_name'); ?>
          </th>        
          <th class="product_price">
            <?php echo JHtml::_('grid.sort', 'Rol Id','rolid', $listDirn, $listOrder); ?>
          </th> 
          <th class="product_price">
            <?php echo JHtml::_('grid.sort', 'Rol','rol', $listDirn, $listOrder); ?>
          </th>        
        </thead>
        <tbody>
          <?php foreach ($this->items as $i => $item):?>      
            <tr class="row<?php echo $i % 2; ?>" sortable-group-id="1"> 
              <?php $rel = CatalogHelper::getIdProductXPrfil($item->id);?>
              <td>                
